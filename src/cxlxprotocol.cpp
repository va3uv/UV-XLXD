...
void CXlxProtocol::HandlePeerLinks(void)
{
    ...
    for ( int i = 0; i < list->size(); i++ )
    {
        ...
        if ( cs[0] == 'X' && cs[1] == 'R' && cs[2] == 'F' ) {
            // Only send DEXTRA connect for XRF peers (skip XLX connect)
            // Place DEXTRA connect logic here if needed
            continue;
        }
...
bool CXlxProtocol::IsValidDvFramePacket(const CBuffer &Buffer, CDvFramePacket* &dvframe)
{
    dvframe = nullptr;
    if ( CDextraProtocol::IsValidDvFramePacket(Buffer, dvframe) )
        return true;
    // otherwise try protocol revision 2
    if ( (Buffer.size() == 45) && (Buffer.Compare((uint8 *)"DSVT", 4) == 0) &&
...
bool CXlxProtocol::IsValidDvLastFramePacket(const CBuffer &Buffer, CDvLastFramePacket* &dvframe)
{
    dvframe = nullptr;
